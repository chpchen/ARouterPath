 ...
 * @Package:com.cyp.p.zAPI.utils.common
 * @author: Timothy
 * @date:2013-7-12 下午5:25:00
 * @version V1.0
 */
...
public class TimeUtils {
	 ...
	 * 各常量允许的最大值
	 */
	private final int[] maxFields = { 59, 59, 1000, Integer.MAX_VALUE - 1 };
	...
	/**
	 * 使用一个字符串构造时间 Time time = new Time("14:22:23");
	 * 
	 * @param time
	 *            字符串格式的时间，默认采用“:”作为分隔符
	 */
	...
	/**
	 * 使用一个字符串构造时间，指定分隔符 Time time = new Time("14-22-23", "-");
	 * 
	 * @param time
	 *            字符串格式的时间
	 */
	...
	/**
	 * @Description: 返回用秒来表示的时间,忽略日期值
	 * @return int 不是时间的1000倍数,就只是秒数
	 * @throws
	 */
	public int getSecondTime() {
		int i = fields[SECOND] + fields[MINUTE] * 60 + fields[HOUR] * 60 * 60;
		return i;
	}
	...
	public String getHour() {
		...
		return timeString.concat(String.valueOf(fields[HOUR]));
	}
	...
	public String getMinute() {
		...
		return timeString.concat(String.valueOf(fields[MINUTE]));
	}
	...
	public String getSecond() {
		...
		return timeString.concat(String.valueOf(fields[SECOND]));
	}
	...
	/**
	 * 时间的处理,将秒转换成时分秒形式,例如 61--->00：01：01
	 * 
	 * @param time
	 * @return
	 */
	public static String formatMillion(int time) {
		if (time <= 0) {
			return "00:00:00";
		}
		int hour = time / 60 / 60;
		String secondnd = (time - hour * 3600) / 60 + "";
		String million = time % 60 + "";
		return addSize(hour + "") + ":" + addSize(secondnd) + ":"
				+ addSize(million);
	}

	private static String addSize(String x) {
